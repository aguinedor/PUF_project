...
--
     ...
     if((SIM_DEVICE = "7SERIES") or (SIM_DEVICE = "7series")) then
         ...
         FIRST_TIME := false;
        elsif((SIM_DEVICE = "VERSAL") or (SIM_DEVICE = "versal")) then
         FIRST_TIME := false;
        elsif((SIM_DEVICE = "VERSAL_ES1") or (SIM_DEVICE = "versal_es1")) then
         FIRST_TIME := false;
        elsif((SIM_DEVICE = "VERSAL_ES2") or (SIM_DEVICE = "versal_es2")) then
         FIRST_TIME := false;	 
        else
          assert FALSE report "Attribute Syntax Error : The Legal values for SIM_DEVICE are 7SERIES or ULTRASCALE or VERSAL or VERSAL_ES1 or VERSAL_ES2." severity Failure;
        end if;
 ...
 out_b_val <= 'X' when (SIM_DEVICE = "ULTRASCALE") else
  ...
  prcs_output : process (IBUFDISABLE, IO, IOB, I, TM, TS,DQS_BIAS_BINARY)
